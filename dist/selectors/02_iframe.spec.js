...
(0, test_1.test)("Výpis všech načtených lokátorů na stránce", async ({ page }) => {
    // Výpis všech lokátorů na stránce
    const frames = page.frames();
    console.log(`Počet iframe na stránce: ${frames.length}`);
    frames.forEach(frame => console.log(`URL iframe: ${frame.url()}`, console.log(`Název iframe: ${frame.name()}`)));
});
(0, test_1.test)("Možnosti vyhledání iframe na stránce", async ({ page }) => {
...
(0, test_1.test)("Problematika nestovaných iframes", async ({ page }) => {
...
(0, test_1.test)("Iterace přes všechny iframe při hledání tlačítka s id 'start'", async ({ page }) => {
    const frames = page.frames();
...
//# sourceMappingURL=02_iframe.spec.js.map